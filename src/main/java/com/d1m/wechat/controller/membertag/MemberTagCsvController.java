...
public class MemberTagCsvController extends BaseController {
	...
	public BaseResponse csvExcelTest(Integer fileId) {
		try {
			...
			return BaseResponse.builder().resultCode(Message.CSV_TEST_SUCCESS.getCode())
					.msg(Message.CSV_TEST_SUCCESS.getName()).build();
		} catch (Exception e) {
			log.error(e.getMessage(), e);
			return BaseResponse.builder().resultCode(Message.CSV_TEST_FAIL.getCode())
	...
	public BaseResponse batchAddTagsOnCsv(@RequestParam MultipartFile file) throws Exception {
		...
		final MemberTagCsv.MemberTagCsvBuilder memberTagCsvBuilder = MemberTagCsv.builder()
		...
		try {
			...
			memberTagCsvService.updateByPrimaryKeySelective(MemberTagCsv.builder().fileId(memberTagCsv.getFileId())
					.status(MemberTagCsvStatus.IMPORT_FAILURE).errorMsg(e.getMessage()).build());

			return BaseResponse.builder().resultCode(Message.CSV_OR_EXCEL_PARSER_FAIL.getCode()).msg(e.getMessage())
					.build();
		}
		return BaseResponse.builder().resultCode(Message.FILE_UPLOAD_SUCCESS.getCode())
	...
	public BaseResponse failDataExport(@PathVariable Integer id, HttpServletResponse response) {
		...
		if (Objects.isNull(memberTagCsv)) {
			return BaseResponse.builder().resultCode(0).msg("找不到上传文件").build();
		}
		...
		if (CollectionUtils.isEmpty(memberTagDatas)) {
			return BaseResponse.builder().resultCode(0).msg("没有错误数据").build();
		}
	...
	public BaseResponse successDataExport(@PathVariable Integer id, HttpServletResponse response) {
		...
		final List<MemberTagData> memberTagDatas = memberTagDataService.selectByExample(example);
		if (CollectionUtils.isEmpty(memberTagDatas)) {
			return BaseResponse.builder().resultCode(0).msg("没有数据").build();
		}
