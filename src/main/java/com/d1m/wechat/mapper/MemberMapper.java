...
public interface MemberMapper extends MyMapper<Member> {
	...
	public List<ExcelMember> totalMember(Integer wechatId,
			...
			Byte sex, Integer country,
			Integer province, Integer city,
			Boolean subscribe,
			Integer activityStartAt,
			Integer activityEndAt,
			Integer batchSendOfMonthStartAt,
			Integer batchSendOfMonthEndAt,
			Date attentionStartAt,
			Date attentionEndAt,
			Date cancelSubscribeStartAt,
			Date cancelSubscribeEndAt,
			Boolean isOnline,
			String fromWhere,
			String mobile,
			Integer[] memberTags,
			String sortName, String sortDir,
			Integer bindStatus, Date daytime);
}
